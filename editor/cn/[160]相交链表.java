...
public class Solution {
    public ListNode getIntersectionNode(ListNode headA, ListNode headB) {
        ...
        // 取差值，然后将长出来的链表，先移动多的步，将长出来的切掉
        int lengthDelta = lengthA - lengthB;
        node = lengthDelta > 0 ? headA : headB;
        ListNode temp = lengthDelta > 0 ? headB : headA;
        ...
        // 现在两个链表相对于交汇点的距离相等
        while (Objects.nonNull(node) && Objects.nonNull(temp)) {
            if (temp == node) {
                return node;
            }
            node = node.next;
